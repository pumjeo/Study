/* 나만의 문자열 클래스 만들기 - 늘품실습 */

#define _CRT_SECURE_NO_WARNINGS
#include <iostream>
#include <string.h> // strlen 때문에 썼지 사실 직접 만들어도 된다.
...
MyString& MyString::insert(int loc, const MyString& str) {
	...
	// abc라는 문자열에 insert(1, "d")를 하면 adbc가 된다.
...
MyString& MyString::erase(int loc, int num) {
	...
	// 지운다는 것은 단순히 뒤의 문자들을 앞을 끌고 온다고 생각
...
int MyString::compare(const MyString& str) const {
	// (*this)-(str)을 수행해서 그 1, 0, -1로 결과를 리턴한다.
	// 1은 (*this)가 사전식으로 더 뒤에 온다는 의미, 0은 두 문자열
	// 이 같다는 의미, -1은 (*this)가 사전식으로 더 앞에 온다는 의미
...
int main() {
	...
	std::cout << str1.find(0, "c") << std::endl;
}
