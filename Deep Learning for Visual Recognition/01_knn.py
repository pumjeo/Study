...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" with your code (do not modify this line)
    x_train = x_train.view(num_train,-1)
    x_test = x_test.view(num_test,-1).t()

    a = torch.sum(x_train**2,dim=1)
    b = torch.sum(x_test**2,dim=0)
    c = torch.mm(x_train,x_test)

    a = a.reshape(-1,1)
    b = b.reshape(1,-1)

    dists = torch.sqrt(a+b-2*c)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    def mode_custom(x):
      max = torch.max(x)
      bin = torch.zeros(max+1,dtype=torch.int64)
      for i in range(x.shape[0]):
        bin[x[i]] += 1
      mode = torch.argmax(bin)

      return mode

    for i in range(y_pred.shape[0]):
      vals, idxs = torch.topk(dists[:,i],k, largest=False)
      list = y_train[idxs]
      y_pred[i] = mode_custom(list)
...
def knn_cross_validate(
    ...
    for i in k_choices:
      ...
      for j in range(num_folds):
        x_train_folds_crv = torch.cat([x_train_folds[u] for u in range(num_folds) if j!=u],dim=0)
        y_train_folds_crv = torch.cat([y_train_folds[u] for u in range(num_folds) if j!=u],dim=0)
        classifier = KnnClassifier(x_train_folds_crv,y_train_folds_crv)
        acc = classifier.check_accuracy(x_train_folds[j], y_train_folds[j],k=i)
        list.append(acc)
      k_to_accuracies[i]=list
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k,accs in k_to_accuracies.items():
      k_to_accuracies[k] = torch.mean(torch.tensor(accs))

    best_item = max(k_to_accuracies.items(), key=lambda x : x[1])
    best_k = best_item[0]
