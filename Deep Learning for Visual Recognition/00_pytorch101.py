...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    for index,indexval in enumerate(indices):
      x[indexval] = values[index]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def count_tensor_elements(x: Tensor) -> int:
    ...
    You should not use the functions torch.dim, torch.numel, or their
    instance method variants x.dim, x.numel.
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" with your code (do not modify this line)
    if stop-start >= 9:
      x = torch.arange(start+10-start%10, stop+1, 10)
    else:
      x = torch.empty(0)
...
def slice_indexing_practice(x: Tensor) \
    ...
    Returns:
        A tuple of:
        - last_row: Tensor of shape (N,) giving the last row of x. It should
          be a one-dimensional tensor.
        - third_col: Tensor of shape (M, 1) giving the third column (index 2)
          of x. It should be a two-dimensional tensor.
        - first_two_rows_three_cols: Tensor of shape (2, 3) giving the data in
          the first two rows and first three columns of x.
        - even_rows_odd_cols: Two-dimensional tensor containing the elements
          in the even-valued rows and odd-valued columns of x.
    """
    assert x.shape[0] >= 3
    assert x.shape[1] >= 5
    last_row = None
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    - You should only mutate the tensor using slice assignment operations,
      where you assign an **integer** to a slice of the tensor.
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    Your implementation should construct the output tensor using a single
    integer array indexing operation. The input tensor should not be modified.
    Your implementation should not use torch.flip.
    ...
    Returns:
        y: Tensor of shape (M, N) which is the same as x but with the rows
            reversed - the first row of y should be equal to the last row of
            x, the second row of y should be equal to the second to last row
            of x, and so on.
    """
    y = None
    ##########################################################################
    ...
    idx_rev = sorted(idx,reverse=True)
    y = x[idx_rev]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    Returns:
        ...
        - The first element is the second element of the first column of x
        - The second element is the first element of the second column of x
        - The third element is the fourth element of the third column of x
    """
    y = None
    ##########################################################################
    #                      TODO: Implement this function                     #
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    Returns:
        y: Tensor of shape (N, C) and where C = 1 + max(x) is one more than
            the max value in x. The n-th row of y is a one-hot-vector
            representation of x[n]; in other words, if x[n] = c then
            y[n, c] = 1; all other elements of y are zeros. The dtype of y
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    the data of the input tensor using only a single comparison operation and
    a single indexing operation. You should not use the Python native sum, but
    the PyTorch function torch.sum or its instance method variant x.sum.
...
def zero_row_min(x: Tensor) -> Tensor:
    """
    Return a copy of the input tensor x, where the minimum value along each
    row has been set to 0.
    ...
    Your implementation should use reduction and indexing operations. You
    should not use any Python loops (including comprehensions). The input
    tensor should not be modified.
    Hint: You can use torch.clone and torch.argmin or their instance
    method variants.
    ...
    Returns:
        y: Tensor of shape (M, N) that is a copy of x, except the minimum
    ...
    ##########################################################################
    # Replace "pass" with your code (do not modify this line)
    vals, idxs1 = x.min(dim=1)
    idxs0 = range(x.shape[0])
    x[idxs0,idxs1] = 0
    y = x
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(x.shape[0]):
      res = torch.mm(x[i],y[i])
      append.append(res)

    z = torch.stack(append)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    dev = x-mean
    dev_sqr = dev**2
    strd = torch.sqrt(torch.sum(dev_sqr,dim=0)/(rownum-1))
